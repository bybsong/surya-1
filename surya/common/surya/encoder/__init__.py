...
class PatchMerger(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if isinstance(self.ln_q, nn.Identity):
...
class VisionAttention(nn.Module):
    def __init__(self, config: SuryaEncoderConfig, dim: int, num_heads: int = 16, bias: bool = True) -> None:
        super().__init__()
...
SURYA_VISION_ATTENTION_CLASSES = {
    "eager": VisionAttention,
    "eager_v2": VisionAttentionV2,
    "flash_attention_2": VisionFlashAttention2,
    "sdpa": None,  # set below after class definition
}
...
# Finish mapping after class is defined
SURYA_VISION_ATTENTION_CLASSES["sdpa"] = VisionSdpaAttention


class SuryaSwiGLUFFN(nn.Module):
    def __init__(self, config: SuryaEncoderConfig):
        super().__init__()
...
class SuryaPatchEmbed(nn.Module):
    def __init__(self, config: SuryaEncoderConfig):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor, grid_thw: Optional[torch.Tensor] = None) -> torch.Tensor:
        x = x.view(
            -1,
            self.num_channels,
        ...
        # Match input dtype to conv weight dtype to avoid bf16/float mismatches
...
class SuryaViTPreprocessor(nn.Module):
    def __init__(self, config: SuryaEncoderConfig):
        super().__init__()
        ...
        self.embed_dim = config.embed_dim
        self.config = config
        self.patchifier = SuryaPatchEmbed(config)
...
class SuryaVisionBlock(nn.Module):
    def __init__(self, config: SuryaEncoderConfig, attn_implementation: str = "flash_attention_2"):
        super().__init__()
...
class SuryaVisionTransformerPreTrainedModel(PreTrainedModel):
    config_class = SuryaEncoderConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["SuryaVisionBlock"]
...
class SuryaVisionTransformer(SuryaVisionTransformerPreTrainedModel):
    config_class = SuryaEncoderConfig

    def __init__(self, config: SuryaEncoderConfig, *inputs, **kwargs) -> None:
        super().__init__(config, *inputs, **kwargs)
        self.config = config
        self.spatial_merge_size = config.spatial_merge_size

        self.patch_embed = SuryaViTPreprocessor(config)
        ...
        self.blocks = nn.ModuleList([SuryaVisionBlock(config) for _ in range(_num_hidden_layers)])
    ...
    def get_pos_ids_by_grid(self, grid_thw: torch.Tensor) -> torch.Tensor:
        pos_ids = []
...
class SuryaEncoderModel(SuryaVisionTransformer):
    @property
